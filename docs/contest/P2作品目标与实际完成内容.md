# P2–P4 作品材料　｜　架构与闭环　｜　P13–P16 代表性案例两轮

> 本稿供报告撰写使用：P2–P12 为总体；P13–P16 把 V26.5 第 1→2 轮讲透。  
...
**第一，一次性生成完整实验方案，执行结果回不来。** 大模型可以根据题目一次性写出假设、对照表、超参网格和评测指标，看起来像一份完整计划，但计划本身不负责跑实验，也不负责把真实读数接回下一轮。下一轮若要继续，往往只能重新提示一次，或把上一轮日志摘要塞进上下文。这样得到的是「又一份方案」，不是「由证据驱动的下一枪」。方案文本可以很长、很完整，却无法回答：上一轮究竟跑成了什么、哪一条假设被证伪、下一轮为什么必须改这一处而不是另一处。
...
本作品针对上述四项中的**全部四项**，核心只做一件事：把实验从「一次性生成方案」改成「在冻结约束下，由真实执行结果驱动的多轮计划调整」。具体希望补上的能力是：
...
- **Planner 只决定试什么、为什么试（WHAT / WHY）**：输出结构化 `ExperimentPlan`（假设、候选实验、本轮选定、预期效应、决策摘要）。计划必须带 `memory_refs` / `evidence_runs`（第二轮起），不能只把历史粘进提示词。
...
- **相较一次性生成实验方案，本作品最主要的不同：**  
  一次性方案生成的产物是一份开场写完的实验菜单，执行与否、下一轮如何改，都不在同一套合约里。本作品把「计划」做成必须被执行、必须被读数、必须被判断、必须被下一轮引用的对象：下一轮计划不是重新生成一份更长的方案，而是根据上一轮真实 GPU 结果、规则判断和记忆编号重建出来的。约束冻结保证尺子不变；空转与混杂对照会被拦住；「值得继续」与「已经证明」被拆开。因此迭代的单位是一轮可审计的科研动作，而不是又一次文本生成。
...
| **根据实验结果决定继续、调整或停止** | **继续（KEEP / REPLICATE）**：主指标相对比较锚未明显变差，证据有效 → 保留 commit，可换种子复现，或在同一假设上收紧对照。**调整**：DISCARD 后禁止下一轮再选被丢弃的修改范围；空转（同 HOW+同 seed）被门禁拦住；人可用 steer 给下一轮方向，但不能改冻结切片/指标。方法空间不够时，调整为发明新 HOW（草稿→人审→冒烟→overlay），而不是改协议。**停止**：触发 `stop_rules`（最大轮次、连续 DISCARD、连续无提升、重复计划被拒、执行失败上限）；或科学上证据不足且再打一枪信息增益低（如 P4 迁移探针 STOP / INCONCLUSIVE）；或产品问题已回答（如 P0 两轮无人值守后停）。停止时写裁决文件，区分产品门通过与科学结论。人若要改尺子，必须协议修订，`protocol_version+1`，新旧结果不得静默混比。 | 没有停机规则的迭代会变成刷分。把「继续 / 调整 / 停止」绑在证据状态和协议上，才能在预算内结束一条研究链，并如实留下「不确定」而不是硬写成成功或失败。 |
...
- **KEEP 且对照公平：** 下一轮优先做更强检验——换种子复现、保持单变量、或在方法空间不够时走发明链（新 HOW 草稿）。下一轮计划必须引用本轮 `evidence_runs` 与 lesson / strategy ID。
- **DISCARD：** 下一轮必须改向：换假设或换可编辑变量；被丢弃范围不能装作新发现再选一次。负结果仍写入记忆，作为「不要重复这条」。
- **REPLICATE / 空转被抓住：** 下一轮必须 bump seed 或换 HOW，不能靠同一合同刷出第二条曲线。
...
**根据实验结果决定继续、调整或停止：** KEEP 则复现或收紧对照；DISCARD 则改向；空转拦住；触达停机规则或信息增益不足则 STOP 并写裁决。理由是迭代必须能在预算内诚实结束。
...
| **反馈与下一轮计划调整** | 本轮决策、lesson/strategy 草稿、evidence run id；可选人 steer；停机规则计数。**Qwen Reviewer** 提议结构；**MemoryWriter** 落盘。 | 教训必须绑定 run。`next_plan` 强制引用这些 ID。KEEP → 复现或收紧对照；DISCARD → 改向且不得重选丢弃范围；空转 → bump seed；INCONCLUSIVE → 补成对对照或 STOP。触达 `max_rounds` 等则停并写裁决。发明链：草稿→人审→Qwen 写插件→smoke→overlay，下一轮规划才能选它。 | 可引用的 Memory；下一份 `ExperimentPlan` 或 STOP 裁决。 | 下一份计划回到「实验任务规划」入口，形成闭环。**返回的是真实读数与编号，不是聊天摘要。** 若 STOP，不再流入执行模块。 |
...
5. **空转和混杂回不了「新计划」。** 同 HOW+同 seed、多变量齐改，会被拦住或标成无效对照，避免一次性菜单里的第 2、第 3 步被当成已经迭代。

所以区别不在「会不会列出后续实验」，而在：**后续实验必须等真实结果返回后才能合法生成。**
...
| `APS_lowlight`、次级指标、Δ vs R0/上一枪 | **数据分析与判断** → 再进入 **规划** | 写入 observation；`expected_effect` 是否被满足，决定 KEEP 后复现还是 DISCARD 后换 HOW |
| Rubric：KEEP / DISCARD / REPLICATE / INCONCLUSIVE | **反馈** → **规划** | KEEP：同假设换种子或收紧对照；DISCARD：禁止再选该 `modification_scope`；REPLICATE：必须 bump seed；INCONCLUSIVE：补 matched pair 或 STOP |
| ClaimGate BLOCKED / C0 | **判断**（封口） | 下一轮计划仍是探索任务，不能写成「验证已发表结论」 |
| lesson_id / strategy_id / evidence run id | **规划**（强制引用） | Next Plan 的 `memory_refs` 指向这些 ID；无引用则 Round≥1 不合法 |
| 空转 / 重复计划被拒 | **规划 / Gate** | 下一轮必须换 HOW 或 seed，不能靠再生成同一份方案 |
...
4. **空转与假复现：** 同 HOW+同 seed 禁止；REPLICATE 必须 `seed = last_seed+1`（针对 P0 两轮都 F1/42 的问题后来收紧）。
...
| **同 HOW + 同 seed（空转）** | Gate / Planner 合约：上一枪已跑的 HOW+seed 再出现则拒绝或强制 `seed+1`。P0 两轮曾都是 F1/42，产品闭环通过、科学 INCONCLUSIVE；此后写入硬规则。 | 两次分数差 **不能**当方法进步。P0 Rubric 为 REPLICATE，Δ 不能当发现。 | 下一轮必须换 HOW 或 bump seed；不能靠再生成同一合同开训。 | 防止把重复执行包装成迭代。这是对已观测错误的反馈，不是提示词里写一句「请勿重复」。 |
...
*文档对应：Scientist Lab v2.6；代表性案例 V26.5 R1–R2。KEEP 不等于科学声称。不要把插件 P3 或第 3 轮 seed 43 写进本案例的「第二轮调整」。*
