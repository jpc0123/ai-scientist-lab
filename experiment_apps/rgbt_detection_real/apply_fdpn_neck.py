...
def apply_neck(
    ...
    cfg = neck_config
    plugin_neck = str(neck_config.type or "").lower().startswith("plugin:")
    if (neck_config.type == "fdpn" or plugin_neck) and hasattr(existing, "in_channels"):
    ...
    if isinstance(new_encoder, FDPN) or plugin_neck:
        if hasattr(new_encoder, "out_channels"):
            summary["out_channels"] = list(new_encoder.out_channels)
        if hasattr(new_encoder, "out_strides"):
