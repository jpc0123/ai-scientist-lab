...
def as_matcher_indices(matched):
    """D-FINE HungarianMatcher returns {'indices': ...}; upstream RT-DETR returned the list."""
...
class RTDETRCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        D-FINE det_engine passes epoch= in metas; RT-DETR loss ignores it.
        """
        outputs_without_aux = {k: v for k, v in outputs.items() if 'aux' not in k}

        # Retrieve the matching between the outputs of the last layer and the targets
        indices = as_matcher_indices(self.matcher(outputs_without_aux, targets))
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                indices = as_matcher_indices(self.matcher(aux_outputs, targets))
                for loss in self.losses:
                    if loss == 'masks':
